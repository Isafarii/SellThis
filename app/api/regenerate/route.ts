...
export async function POST(request: Request) {
  try {
    ...
    try {
      ...
      return Response.json(buildResult(analysis, listings, !process.env.GEMINI_API_KEY, process.env.GEMINI_API_KEY ? null : "AI is unavailable, so the updated listing was generated from your corrected details.", details));
    } catch {
      return Response.json(buildResult(analysis, deterministicListings(analysis, details), true, "AI is temporarily unavailable, so the updated listing was generated from your corrected details.", details));
    }
